...
// Para que no se cachee y cambie el Pokémon en cada request:
export const dynamic = "force-dynamic";

async function getPokemon(): Promise<Pokemon> {
  const randomId = Math.floor(Math.random() * 150) + 1;
  const res = await axios.get<Pokemon>(`https://pokeapi.co/api/v2/pokemon/${randomId}`);
  return res.data;
}
        ...
        <div className="bg-white rounded-b-xl shadow-2xl p-8 border-4 border-black border-t-0">
          <div className="text-center">
            ...
            <div className="flex gap-2 justify-center mt-4">
              {pokemon.types.map((t) => (
                <span key={t.type.name} className="px-4 py-2 bg-green-500 text-white rounded-full font-semibold">
                  {t.type.name}
                </span>
              ))}
            </div>
          ...
          <div className="mt-8 p-4 bg-green-50 rounded-lg border-2 border-green-200">
            <p className="text-sm text-green-800">
              💡 <strong>SSR:</strong> los datos se obtienen en el servidor antes de enviar el HTML. Revisa “Ver
              código fuente” para comprobarlo.
            </p>
          </div>
        </div>
      </div>
